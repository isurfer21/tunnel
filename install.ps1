...
if ($IsWindows) {
  ...
  Write-Output "Tunnel was installed successfully to $ABSOLUTE_FILEPATH"
  Write-Output "Run 'tunnel --help' to get started"
} else {
  chmod +x "$INSTALLATION_DIR/tunnel"
  Write-Output "Tunnel was installed successfully to $ABSOLUTE_FILEPATH"
  if (Get-Command tunnel -ErrorAction SilentlyContinue) {
    Write-Output "Run 'tunnel --help' to get started"
  } else {
